...
//						}
						...
						if (maxIdle > poolConfig.getMaxTotal()) {
...
//						poolConfig.setNumTestsPerEvictionRun(-1); //-1,表示在驱逐线程执行时,测试所有的空闲对象
	...
	Object deserialize(byte[] bb) throws Exception, ClassNotFoundException {
			...
			try {
				...
				if (jedis != null) {
					try {
						...
						log.error("return resource in jedisExists:", thex);
					}
				}
				throw new JedisConnectionException("jedisExists",e);
			} finally {
				if (success&&jedis != null) {
					try {
						_pool.returnResource(jedis);
					} catch (Throwable thex) {
						log.error("return resource :", thex);
					}
				}
			...
			try {
				...
				if (jedis != null) {
					try {
						...
						log.error("return resource in jedisExpire:", thex);
					}
				}
				throw new JedisConnectionException("jedisExpire",e);
			} finally {
				if (success&&jedis != null) {
					try {
						log.info("return resource for expire.");
						_pool.returnResource(jedis);
					} catch (Throwable thex) {
						log.error("return resource :", thex);
					}
	...
	java.util.List<Object> jedisPipelined(String key, PipelineBlock pipelineBlock) {
		if (_pool != null) {
			...
			try {
				...
				throw new JedisConnectionException("jedisPipelined",e);
			} finally {
				if (success&&jedis != null) {
					try {
						_pool.returnResource(jedis);
	...
	byte[] jedisHget(String hkey, String field) {
		if (_pool != null) {
			...
			boolean success=true;
			try {
				log.error("info:---numActive:"+_pool.getNumActive());
				jedis = _pool.getResource();
				...
				if (jedis != null) {
					try {
						...
						log.error("return resource in jedisHget:", thex);
					}
				}
				throw new JedisConnectionException("jedisHget",e);
			} finally {
				if (success&&jedis != null) {
					try {
						_pool.returnResource(jedis);
	...
	public Long jedisHset(String hkey, String field, byte[] value) {
		if (_pool != null) {
			...
			try {
				...
				throw new JedisConnectionException("jedisHset",e);
			} finally {
				if (success&&jedis != null) {
					try {
						_pool.returnResource(jedis);
